...
public class UserController {
  ...
  public ResponseEntity<Object> getUserById(@PathVariable Long id) {
    ...
    return new ResponseEntity<>(new ValidationFailureResponse(AuthConstants.USER_ID,
        authValidationFailureCodes.getUserNotExist()), HttpStatus.BAD_REQUEST);
  }
  ...
  public ResponseEntity<Object> deleteUser(@PathVariable Long id) {
    if (userService.isUserExist(id)) {
      ...
      return new ResponseEntity<>(
          new BasicResponse<>(RestApiResponseStatus.OK, AuthConstants.DELETE_USER_SCCESS),
          HttpStatus.OK);
    }
    logger.debug("No User record exist for given id");
    return new ResponseEntity<>(new ValidationFailureResponse(AuthConstants.USER_ID,
  ...
  public ResponseEntity<Object> updateUser(@Valid @RequestBody UserRequestDto userRequestDto) {
    if (userService.isUserExist(userRequestDto.getId())) {
      ...
      return new ResponseEntity<>(
          new BasicResponse<>(RestApiResponseStatus.OK, AuthConstants.UPDATE_USER_SUCCESS),
          HttpStatus.OK);
    }
    logger.debug("No User record exist for given id");
    return new ResponseEntity<>(new ValidationFailureResponse(AuthConstants.USER_ID,
