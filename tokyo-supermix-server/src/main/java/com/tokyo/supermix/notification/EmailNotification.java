...
public class EmailNotification {
	...
	public void notifyStrengthTestForMixdesign() {
		...
		finishProductSampleRepository.findAll().forEach(finishProductSample -> {
			long noOfDays = ChronoUnit.DAYS.between(finishProductSample.getCreatedAt().toLocalDateTime().toLocalDate(),
					today.toLocalDate());
	...
	private void sendMixDesignEmail(FinishProductSample finishProductSample, long noOfDays) {
		String mailBody = "The work order no is " + finishProductSample.getWorkOrderNo() + ", created on "
				+ finishProductSample.getDate() + "." + " reached " + noOfDays + "days " + " Please test" + noOfDays
				+ " days strength.";
		...
		if (emailGroup != null) {
			if (emailGroup.isStatus()) {
				...
				String mailBody = "Finish Product sample created for mix design of " + mixDesign.getCode()
						+ " to the grade" + mixDesign.getTargetGrade() + " and Slump" + mixDesign.getTargetSlump();
				List<String> reciepientList = emailRecipientService.getEmailsByEmailNotificationAndPlantCode(
						emailGroup.getEmailPoints().getName(), emailGroup.getPlant().getCode());
				emailService.sendMailWithFormat(reciepientList.toArray(new String[reciepientList.size()]),
	...
	public void sendCustomerCreationEmail(Customer customer) {
		EmailGroup emailGroup = emailGroupRepository.findByEmailPointsName(MailGroupConstance.CREATE_CUSTOMER);
		if (emailGroup != null) {
			if (emailGroup.isStatus()) {
		...
		if (emailGroup != null) {
			if (emailGroup.isStatus()) {
				...
				String IncomingSample = incomingSampleRepository.findById(processSample.getIncomingSample().getCode())
						.get().getCode();
				String mailBody = "Material Load  is arrived from " + supplierName + " for the Passed Incoming sample "
						+ IncomingSample + ", " + rawMaterialName;
				List<String> reciepientList = emailRecipientService.getEmailsByEmailNotificationAndPlantCode(
