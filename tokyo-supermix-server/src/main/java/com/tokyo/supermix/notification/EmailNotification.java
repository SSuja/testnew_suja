...
public class EmailNotification {
	...
	public void alertForEquipmentCalibration() {
		...
		plantEquipmentCalibrationRepository.findAll().forEach(calibration -> {
			...
			if (noOfDays == 30 || noOfDays == 15) {
				emailService.sendMail(calibration.getEmployee().getEmail(), "Notification : equipment calibration",
	...
	public void notifyTheExpiryDateForAdmixure() {
		...
		processSampleLoadRepository.findAll().forEach(processsampleLoad -> {
			if (processsampleLoad.getProcessSample().getIncomingSample().getRawMaterial().getMaterialSubCategory()
					.getMaterialCategory().getName().equalsIgnoreCase(Constants.ADMIXURE)) {
		...
		rawMaterialRepository.findAll().forEach(material -> {
			...
			if (noOfSamples > 0) {
				emailService.sendMail(mailConstants.getMailIncomingSamplePerDay(),
						...
						"Today " + noOfSamples + " Samples Arrived for the Raw Material : " + material.getName());
			}
		});
	}
	...
	public void notifyStrengthTestForMixdesign() {
		...
		mixDesignRepository.findAll().forEach(mixDesign -> {
			long noOfDays = ChronoUnit.DAYS.between(mixDesign.getDate().toLocalDate(),today.toLocalDate());
			System.out.println(noOfDays);
			if (noOfDays == 6 || noOfDays == 27) {
				String mailBody ="Today , " + noOfDays + " days over in mixdesign creation. the Mixdesign Code is "
						+ mixDesign.getCode() + ", Please test this strength. this mixdesign created on " + mixDesign.getDate()
						+ ". target grade of this mixdesign is " + mixDesign.getTargetGrade();
				emailService.sendMail(mailConstants.getMailalertMixDedisgnTest(), "Notification : About Mixdesign ",mailBody);
				emailService.sendPreConfiguredMail(mailBody);
			}
		});
	}
