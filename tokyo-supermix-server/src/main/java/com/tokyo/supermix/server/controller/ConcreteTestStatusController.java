...
public class ConcreteTestStatusController {
	...
	public ResponseEntity<Object> createConcreteTestStatus(
		...
		if (concreteTestStatusService.isDuplicateEntryExist(concreteTestStatusRequestDto.getConcreteTestTypeId(),
				concreteTestStatusRequestDto.getFinishProductSampleId())) {
			return new ResponseEntity<>(new ValidationFailureResponse(Constants.CONCRETE_TEST_STATUS,
					validationFailureStatusCodes.getConcreteTestStatusAlreadyExist()), HttpStatus.BAD_REQUEST);
		}
	...
	public ResponseEntity<Object> getConcreteTestStatuses() {
		return new ResponseEntity<>(new ContentResponse<>(Constants.CONCRETE_TEST_STATUSES,
				mapper.map(concreteTestStatusService.getAllConcreteTestStatus(), ConcreteTestStatusResponseDto.class),
	...
	public ResponseEntity<Object> getConcreteTestStatusById(@PathVariable Long id) {
		if (concreteTestStatusService.isConcreteTestStatusExists(id)) {
			return new ResponseEntity<>(new ContentResponse<>(Constants.CONCRETE_TEST_STATUS, mapper
					.map(concreteTestStatusService.getConcreteTestStatusById(id), ConcreteTestStatusResponseDto.class),
	...
	public ResponseEntity<Object> updateConcreteTestStatus(
			@RequestBody ConcreteTestStatusRequestDto concreteTestStatusRequestDto) {
		if (concreteTestStatusService.isConcreteTestStatusExists(concreteTestStatusRequestDto.getId())) {
			if (concreteTestStatusService.isDuplicateEntryExist(concreteTestStatusRequestDto.getConcreteTestTypeId(),
					concreteTestStatusRequestDto.getFinishProductSampleId())) {
				return new ResponseEntity<>(
			...
			concreteTestStatusService
					.saveConcreteTestStatus(mapper.map(concreteTestStatusRequestDto, ConcreteTestStatus.class));
			return new ResponseEntity<>(
					new BasicResponse<>(RestApiResponseStatus.OK, Constants.UPDATE_CONCRETE_TEST_STATUS_SUCCESS),
					HttpStatus.OK);
		}
		return new ResponseEntity<>(new ValidationFailureResponse(Constants.CONCRETE_TEST_STATUS_ID,
				validationFailureStatusCodes.getConcreteTestStatusNotExist()), HttpStatus.BAD_REQUEST);
