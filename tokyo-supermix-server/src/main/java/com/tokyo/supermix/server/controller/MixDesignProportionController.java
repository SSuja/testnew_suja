...
public class MixDesignProportionController {
	...
	public ResponseEntity<Object> saveMixDesignProportion(
		...
		mixDesignProportionRequestDtoList.forEach(mixDesignProportionRequestDtoObj -> mixDesignProportionService
				.saveMixDesignProportion(mapper.map(mixDesignProportionRequestDtoObj, MixDesignProportion.class)));
	...
	public ResponseEntity<Object> getAllMixDesignProportions() {
		return new ResponseEntity<Object>(new ContentResponse<>(Constants.MIX_DESIGN_PROPORTIONS, mapper
				.map(mixDesignProportionService.getAllMixDesignProportions(), MixDesignProportionResponseDto.class),
	...
	public ResponseEntity<Object> deleteMixDesignProportion(@PathVariable Long id) {
		...
		return new ResponseEntity<>(new ValidationFailureResponse(Constants.MIX_DESIGN_PROPORTION_ID,
				validationFailureStatusCodes.getMixDesignProportionAlreadyExist()), HttpStatus.BAD_REQUEST);
	}
	...
	public ResponseEntity<Object> updateMixDesignProportion(
		...
		if (mixDesignProportionService.isMixDesignProportionExist(mixDesignProportionRequestDto.getId())) {
			mixDesignProportionService
					.saveMixDesignProportion(mapper.map(mixDesignProportionRequestDto, MixDesignProportion.class));
			return new ResponseEntity<Object>(
					new BasicResponse<>(RestApiResponseStatus.OK, Constants.UPDATE_MIX_DESIGN_PROPORTION_SUCCESS),
					HttpStatus.OK);
		}
		return new ResponseEntity<>(new ValidationFailureResponse(Constants.MIX_DESIGN_PROPORTION,
				validationFailureStatusCodes.getMixDesignProportionAlreadyExist()), HttpStatus.BAD_REQUEST);

	}

	@GetMapping(value = EndpointURI.MIX_DESIGN_PROPORTION_BY_MIX_DESIGN_CODE)
	public ResponseEntity<Object> getMixDesignProportionByMixDesignCode(@PathVariable String mixDesignCode) {
