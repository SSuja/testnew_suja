...
public class UserServiceImpl implements UserService {
	...
	private UserCredentialDto createUserCredentialDto(String userName, String password, String email) {
	...
	private void createUserPlantRoles(List<Long> roles, User user) {
		...
		roles.forEach(roleId -> userPlantRoles.add(new UserPlantRole(user, plantRoleRepository.getOne(roleId))));
	...
	public String generateRandomPassword() {
		List<CharacterRule> rules = Arrays.asList(new CharacterRule(EnglishCharacterData.UpperCase, 2),
				new CharacterRule(EnglishCharacterData.LowerCase, 2), new CharacterRule(EnglishCharacterData.Digit, 4));
	...
	public boolean isUpdatedUserExist(Long id, String userName) {
		if ((!getUserById(id).getUserName().equalsIgnoreCase(userName)) && (isUserNameExist(userName))) {
			return true;
		}
		return false;
	}
	...
	public UserResponseDto getUserDetailById(Long id) {
		UserResponseDto userResponseDto = mapper.map(userRepository.findById(id).get(), UserResponseDto.class);
