...
public class PlantController {
	...
	public ResponseEntity<Object> createPlant(@Valid @RequestBody PlantDto plantDto) {
		...
		return new ResponseEntity<>(new BasicResponse<>(RestApiResponseStatus.OK, Constants.ADD_PLANT_SUCCESS),
				HttpStatus.OK);
	}
	...
	public ResponseEntity<Object> getAllPlants() {
		...
		return new ResponseEntity<>(new ContentResponse<>(Constants.PLANTS, plantDtoList, RestApiResponseStatus.OK),
				null, HttpStatus.OK);
	}
	...
	public ResponseEntity<Object> getPlantByCode(@PathVariable String code) {
		if (plantService.isPlantExist(code)) {
			...
			return new ResponseEntity<>(new ContentResponse<>(Constants.PLANT, mapper.map(plantDto, PlantDto.class),
	...
	public ResponseEntity<Object> updatePlant(@Valid @RequestBody PlantDto plantDto) {
		if (plantService.isPlantExist(plantDto.getCode())) {
			...
			Plant plant = mapper.map(plantDto, Plant.class);
			plantService.savePlant(plant);
			return new ResponseEntity<>(new BasicResponse<>(RestApiResponseStatus.OK, Constants.UPDATE_PLANT_SUCCESS),
					HttpStatus.OK);
		}
		return new ResponseEntity<>(
				new ValidationFailureResponse(Constants.PLANT_NAME, validationFailureStatusCodes.getPlantNotExist()),
				HttpStatus.BAD_REQUEST);
	}
	...
	public ResponseEntity<Object> deletePlant(@PathVariable String code) {
		if (plantService.isPlantExist(code)) {
			...
			return new ResponseEntity<>(new BasicResponse<>(RestApiResponseStatus.OK, Constants.PLANT_DELETED),
					HttpStatus.OK);
		}
