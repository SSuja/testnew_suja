...
public class ConcreteTestTypeController {
	...
	public ResponseEntity<Object> createConcreteTestType(@Valid @RequestBody ConcreteTestTypeDto ConcreteTestTypeDto) {
		...
		return new ResponseEntity<>(
				new BasicResponse<>(RestApiResponseStatus.OK, Constants.ADD_CONCRETE_TEST_TYPE_SUCCESS), HttpStatus.OK);
	}
	...
	public ResponseEntity<Object> getConcreteTestTypeById(@PathVariable Long id) {
		if (concreteTestTypeService.isConcreteTestTypeExists(id)) {
			...
			return new ResponseEntity<>(new ContentResponse<>(Constants.CONCRETE_TEST_TYPE,
					mapper.map(concreteTestTypeService.getConcreteTestTypeById(id), ConcreteTestTypeDto.class),
					RestApiResponseStatus.OK), HttpStatus.OK);
		}
	...
	public ResponseEntity<Object> deleteConcreteTestType(@PathVariable Long id) {
		if (concreteTestTypeService.isConcreteTestTypeExists(id)) {
			...
			return new ResponseEntity<>(
					new BasicResponse<>(RestApiResponseStatus.OK, Constants.CONCRETE_TEST_TYPE_DELETED), HttpStatus.OK);
		}
	...
	public ResponseEntity<Object> updateConcreteTest(@Valid @RequestBody ConcreteTestTypeDto concreteTestTypeDto) {
		if (concreteTestTypeService.isConcreteTestTypeExists(concreteTestTypeDto.getId())) {
			concreteTestTypeService.saveConcreteTestType(mapper.map(concreteTestTypeDto, ConcreteTestType.class));
			return new ResponseEntity<>(
					new BasicResponse<>(RestApiResponseStatus.OK, Constants.UPDATE_CONCRETE_TEST_TYPE_SUCCESS),
					HttpStatus.OK);
		}
		return new ResponseEntity<>(new ValidationFailureResponse(Constants.CONCRETE_TEST_TYPE,
				validationFailureStatusCodes.getConcreteTestTypeNotExist()), HttpStatus.BAD_REQUEST);
