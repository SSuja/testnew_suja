...
public class ConcreteStrengthTestServiceImpl implements ConcreteStrengthTestService {
	...
	public boolean checkConcreteAge(Long concreteAge) {
		if (!(concreteAge == 1 || concreteAge == 3 || concreteAge == 5 || concreteAge == 7 || concreteAge == 14
				|| concreteAge == 21 || concreteAge == 28 || concreteAge == 56 || concreteAge == 128)) {
			return true;
		}
		return false;
	}
	...
	private ConcreteStrengthTest calculateConcreteStrengthRatio(ConcreteStrengthTest concreteStrengthTest) {
		MixDesign mixDesign = mixDesignRepository
				.findById(concreteStrengthTest.getFinishProductSample().getMixDesign().getCode()).get();
		...
		if ((ratio >= 0.16 && concreteAge == 1) || (ratio >= 0.40 && concreteAge == 3)
			...
			String messsage = "Congrete Strength Test is " + concreteStrengthTest.getStatus()
					...
					+ concreteStrengthTest.getFinishProductSample().getMixDesign().getCode() + "<ul><li> Age : "
					+ concreteStrengthTest.getConcreteAge() + " days </li>" + "<li> Strength : "
					+ concreteStrengthTest.getStrength() + "</li></ul>";
