...
public class TestReportServiceImpl implements TestReportService {
  ...
  FinishProductSampleRepository finishProductSampleRepository;
	...
	public AdmixtureTestReportDto getAdmixtureReport(String materialTestCode) {
		...
		admixtureTestReportDto.setIncomingsample(getIncomingSampleDetails(materialTest.getIncomingSample().getCode()));
		admixtureTestReportDto.setTestTrials(getMaterialTestTrialDtoReport(materialTestCode));
		admixtureTestReportDto.setPlant(mapper.map(materialTest.getIncomingSample().getPlant(), PlantDto.class));
		admixtureTestReportDto.setMaterialAcceptedValueDto(getMaterialAcceptedValueDto(
				materialTest.getTestConfigure().getId(), materialTest.getIncomingSample().getRawMaterial().getId()));
		admixtureTestReportDto.setTrailValues(getTrailValueDtoList(materialTestCode));
		return admixtureTestReportDto;
	}

	private MaterialAcceptedValueDto getMaterialAcceptedValueDto(Long testConfigureId, Long rawMaterialId) {
		...
		materialAcceptedValueDto.setTestName(materialAcceptedValue.getTestConfigure().getTest().getName());
	...
	public IncomingSampleDeliveryReportDto getIncomingSampleSummaryReport(String incomingSampleCode) {
		IncomingSampleDeliveryReportDto incomingSampleDeliveryReportDto = new IncomingSampleDeliveryReportDto();
		List<MaterialTest> materialTest = materialTestRepository.findByIncomingSampleCode(incomingSampleCode);
		incomingSampleDeliveryReportDto
				.setIncomingsample(getIncomingSampleDetails(materialTest.get(0).getIncomingSample().getCode()));
		incomingSampleDeliveryReportDto
				.setPlant(mapper.map(materialTest.get(0).getIncomingSample().getPlant(), PlantDto.class));
		incomingSampleDeliveryReportDto.setIncomingSampleTestDtos(getIncomingSampleTestDtoReport(incomingSampleCode));
		incomingSampleDeliveryReportDto.setIncomingSampleStatusCounts(getIncomingSampleStatusCount(incomingSampleCode));
	...
	private List<IncomingSampleTestDto> getIncomingSampleTestDtoReport(String incomingSampleCode) {
		...
		materialTestRepository.findByIncomingSampleCode(incomingSampleCode).forEach(test -> {
			...
			incomingSampleTestDto.setAcceptanceCriteria(getAcceptedCriteriaDetails(test.getTestConfigure().getId()));
			incomingSampleTestDtoList.add(incomingSampleTestDto);
		});
		return incomingSampleTestDtoList;
	}

	private List<IncomingSampleTestDto> getIncomingSampleDeliveryReport(String incomingSampleCode, String testName) {
		List<IncomingSampleTestDto> incomingSampleTestDtoList = new ArrayList<IncomingSampleTestDto>();
		materialTestRepository.findByIncomingSampleCodeAndTestConfigureTestName(incomingSampleCode, testName)
				.forEach(test -> {
					IncomingSampleTestDto incomingSampleTestDto = new IncomingSampleTestDto();
					incomingSampleTestDto.setTestName(test.getTestConfigure().getTest().getName());
					incomingSampleTestDto.setAverage(test.getAverage());
	...
	private SupplierReportDto getSupplierReport(Long supplierId) {
		...
		supplierReportDto.setSupplierCategoryName(supplier.getSupplierCategories().get(0).getCategory());
	...
	private List<IncomingSampleStatusCount> getIncomingSampleStatusCount(String incomingSampleCode) {
		List<IncomingSampleStatusCount> incomingSampleStatusCountList = new ArrayList<IncomingSampleStatusCount>();
		List<MaterialTest> materialTestList = materialTestRepository.findByIncomingSampleCode(incomingSampleCode);
		Status status = null;
		incomingSampleStatusCountList
				.add(setIncomingSampleStatusCount(materialTestList.get(0).getIncomingSample().getCode(), status));
		return incomingSampleStatusCountList;
	}

	private IncomingSampleStatusCount setIncomingSampleStatusCount(String incomingSampleCode, Status status) {
		IncomingSampleStatusCount incomingSampleStatusCount = new IncomingSampleStatusCount();
		incomingSampleStatusCount.setNewCount(
				materialTestRepository.findByIncomingSampleCodeAndStatus(incomingSampleCode, Status.NEW).size());
		incomingSampleStatusCount.setPassCount(
				materialTestRepository.findByIncomingSampleCodeAndStatus(incomingSampleCode, Status.PASS).size());
		incomingSampleStatusCount.setFailCount(
				materialTestRepository.findByIncomingSampleCodeAndStatus(incomingSampleCode, Status.FAIL).size());
		incomingSampleStatusCount.setProcessCount(
				materialTestRepository.findByIncomingSampleCodeAndStatus(incomingSampleCode, Status.PROCESS).size());
	...
	public SieveTestReportDto getSieveTestReport(String materialTestCode) {
		...
		sieveTestReportDto.setIncomingsample(getIncomingSampleDetails(materialTest.getIncomingSample().getCode()));
		sieveTestReportDto.setPlant(mapper.map(materialTest.getIncomingSample().getPlant(), PlantDto.class));
		sieveTestReportDto.setAcceptanceCriteria(getAcceptedCriteriaDetails(materialTest.getTestConfigure().getId()));
		sieveTestReportDto.setTrailValues(getTrailValueDtoList(materialTestCode));
		sieveTestReportDto
				.setSieveSizes(getSieveSizeDto(materialTest.getTestConfigure().getMaterialSubCategory().getId()));
	...
	private List<SieveSizeDto> getSieveSizeDto(Long materialSubCategoryId) {
		...
		sieveAcceptedValueRepository.findBySieveSizeMaterialSubCategoryId(materialSubCategoryId).forEach(sieve -> {
	...
	public ConcreteTestReportDto getConcreteTestReport(String finishProductTestCode) {
		...
		concreteTestReportDto
				.setAddress(finishProductTest.getFinishProductSample().getMixDesign().getPlant().getAddress());
		concreteTestReportDto
				.setPlantName(finishProductTest.getFinishProductSample().getMixDesign().getPlant().getName());
		concreteTestReportDto
				.setFaxNumber(finishProductTest.getFinishProductSample().getMixDesign().getPlant().getFaxNumber());
		concreteTestReportDto.setReportNo(finishProductTest.getFinishProductSample().getFinishProductCode());
		concreteTestReportDto.setCustomerName(finishProductSampleIssue.getProject().getCustomer().getName());
		concreteTestReportDto.setProjectName(finishProductSampleIssue.getProject().getName());
		concreteTestReportDto
				.setTargetGrade(finishProductSampleIssue.getFinishProductSample().getMixDesign().getTargetGrade());
		concreteTestReportDto
				.setTargetSlump(finishProductSampleIssue.getFinishProductSample().getMixDesign().getTargetSlump());
		concreteTestReportDto
				.setDateOfCasting(finishProductSampleIssue.getFinishProductSample().getCreatedAt().toString());
		concreteTestReportDto
				.setDateOfTesting(finishProductSampleIssue.getFinishProductSample().getUpdatedAt().toString());
	...
	@Transactional(readOnly = true)
	public List<MaterialTestTrialResultDto> getMaterialTestTrailByMaterialTestCode(String materialTestCode) {
		List<MaterialTestTrial> materialTestTrialList = materialTestTrialRepository
		...
		ArrayList<MaterialTestTrialResultDto> materialTestTrialResultDtoList = new ArrayList<MaterialTestTrialResultDto>();
		for (ParameterResult parameterResult : parameterResultList) {
			...
			materialTestTrialResultDto.setAbbrevation(parameterResult.getTestParameter().getAbbreviation());
	...
	public List<AbbrevationAndValueDto> getAbbAndValueByMaterialTestCode(String materialTestCode,
		...
		List<MaterialTestTrial> materialTestTrialList = materialTestTrialRepository
				.findByMaterialTestCode(materialTestCode);
		ArrayList<AbbrevationAndValueDto> abbrevationAndValueDtoList = new ArrayList<AbbrevationAndValueDto>();
		for (MaterialTestTrial materialTestTrial : materialTestTrialList) {
			...
			ParameterResult parameterResult = parameterResultRepository
					.findByTestParameterIdAndMaterialTestTrialCode(testParameterId, materialTestTrial.getCode());
