...
public class TestReportServiceImpl implements TestReportService {
	...
	public TestReportDetailDto getMaterialTestDetailReport(String materialTestCode) {
		...
		List<TestEquation> testEquation = testEquationRepository
				.findByTestConfigureId(materialTest.getTestConfigure().getId());
		List<MaterialTestResult> materialTestResult = materialTestResultRepository
		...
		if ((materialTest.getTestConfigure().getMaterialCategory().getName().equalsIgnoreCase("Admixture"))) {
			reportDto.setAcceptanceCriteria(getMaterialAcceptedValueDto(materialTest.getTestConfigure().getId(),
	...
	private List<IncomingSampleTestDto> getIncomingSampleTestDtoReport(String incomingSampleCode) {
		...
		materialTestRepository.findByIncomingSampleCode(incomingSampleCode).forEach(test -> {
			...
			List<MaterialTestResult> materialTestResult = materialTestResultRepository
					.findByMaterialTestCode(test.getCode());
			...
			if ((test.getTestConfigure().getMaterialCategory().getName().equalsIgnoreCase("Admixture"))) {
				incomingSampleTestDto.setAcceptanceCriteria(getMaterialAcceptedValueDto(test.getTestConfigure().getId(),
	...
	public ConcreteTestReportDto getConcreteTestReport(String finishProductTestCode) {
		...
		if (!finishProductTest.getTestConfigure().isCoreTest()) {
			...
			if (finishProductTest.getFinishProductSample().getProject() != null) {
				...
				concreteTestReportDto.setProjectName(finishProductTest.getFinishProductSample().getProject().getName());
			}
			concreteTestReportDto
