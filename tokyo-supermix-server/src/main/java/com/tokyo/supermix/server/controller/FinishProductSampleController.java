...
public class FinishProductSampleController {
  ...
  public ResponseEntity<Object> createFinishProductSample(
    ...
    finishProductSampleService.saveFinishProductSample(mapper.map(finishProductSampleRequestDto, FinishProductSample.class));
    return new ResponseEntity<>(new BasicResponse<>(RestApiResponseStatus.OK, Constants.ADD_FINISH_PRODUCT_SAMPLE_SUCCESS),
        HttpStatus.OK);
  }
  ...
  public ResponseEntity<Object> getAllFinishProductSamples() {
    ...
    return new ResponseEntity<>(
        new ContentResponse<>(Constants.FINISH_PRODUCT_SAMPLES, mapper.map(finishProductSampleService.getAllFinishProductSamples(),
                FinishProductSampleResponseDto.class), RestApiResponseStatus.OK), null, HttpStatus.OK);
  }
  ...
  public ResponseEntity<Object> getFinishProductSampleById(@PathVariable Long id) {
    if (finishProductSampleService.isFinishProductSampleExist(id)) {
      ...
      return new ResponseEntity<>(new ContentResponse<>(Constants.FINISH_PRODUCT_SAMPLE,
          mapper.map(finishProductSampleService.getFinishProductSampleById(id), FinishProductSampleResponseDto.class),
  ...
  public ResponseEntity<Object> deleteFinishProductSample(@PathVariable Long id) {
    if (finishProductSampleService.isFinishProductSampleExist(id)) {
      ...
      return new ResponseEntity<>(
          new BasicResponse<>(RestApiResponseStatus.OK, Constants.FINISH_PRODUCT_SAMPLE_DELETED), HttpStatus.OK);
    }
    return new ResponseEntity<>(new ValidationFailureResponse(Constants.FINISH_PRODUCT_SAMPLE_ID,
        validationFailureStatusCodes.getFinishProductSampleNotExist()), HttpStatus.BAD_REQUEST);
  }
  ...
  public ResponseEntity<Object> updateFinishProductSample(@Valid @RequestBody FinishProductSampleRequestDto finishProductSampleRequestDto) {
    if (finishProductSampleService.isFinishProductSampleExist(finishProductSampleRequestDto.getId())) {
      if (finishProductSampleService.isUpdatedFinishProductCodeExist(finishProductSampleRequestDto.getId(),
          finishProductSampleRequestDto.getFinishProductCode())) {
        return new ResponseEntity<>(new ValidationFailureResponse(Constants.FINISH_PRODUCT_CODE,
            validationFailureStatusCodes.getFinishProductSampleAlreadyExist()), HttpStatus.BAD_REQUEST);
      }
      finishProductSampleService.saveFinishProductSample(mapper.map(finishProductSampleRequestDto, FinishProductSample.class));
      return new ResponseEntity<>(
          new BasicResponse<>(RestApiResponseStatus.OK, Constants.UPDATE_FINISH_PRODUCT_SAMPLE_SUCCESS),
          HttpStatus.OK);
    }
    return new ResponseEntity<>(new ValidationFailureResponse(Constants.FINISH_PRODUCT_SAMPLE_ID,
        validationFailureStatusCodes.getFinishProductSampleAlreadyExist()), HttpStatus.BAD_REQUEST);
  ...
  public ResponseEntity<Object> getFinishProductSampleByMixDesignCode(@PathVariable String mixDesignCode) {
    if (finishProductSampleService.isMixDesignCodeExist(mixDesignCode)) {
      ...
      return new ResponseEntity<>(new ContentResponse<>(Constants.FINISH_PRODUCT_SAMPLES,
          mapper.map(finishProductSampleService.getFinishProductSampleByMixDesignCode(mixDesignCode), FinishProductSampleResponseDto.class),
          RestApiResponseStatus.OK), HttpStatus.OK);
    }
  ...
  public ResponseEntity<Object> getFinishProductSampleByConcreteMixerId(@PathVariable Long concreteMixerId) {
