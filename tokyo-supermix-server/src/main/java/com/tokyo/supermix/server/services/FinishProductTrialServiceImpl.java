...
public class FinishProductTrialServiceImpl implements FinishProductTrialService {
	...
	public void updateFinishProductSampleStatus(String finishProductSampleCode, Status status) {
		FinishProductSample finishProductSample = finishProductSampleRepository.findById(finishProductSampleCode).get();
	...
	@Transactional(readOnly = true)
	public List<FinishProductTrial> getFinishProductTrialsByFinishProductTestCode(String finishProductTestCode) {
		return finishProductTrialRepository.findByFinishProductTestCodeOrderByCreatedAtDesc(finishProductTestCode);
	}
	...
	public List<FinishProductTrial> getAllFinishProductTrialsByPlant(UserPrincipal currentUser) {
		return finishProductTrialRepository.findByFinishProductTestFinishProductSampleMixDesignPlantCodeIn(
	...
	public List<FinishProductTrial> getAllFinishProductTrialsByPlantCode(String plantCode) {
		return finishProductTrialRepository.findByFinishProductTestFinishProductSampleMixDesignPlantCode(plantCode);
	}
	...
	public void saveFinishproductResult(String finishProductTestCode, HttpServletRequest request) {
		...
		TestConfigure testConfigure = testConfigureRepository.findById(finishproductTest.getTestConfigure().getId())
				.get();
		List<TestParameter> testParameterList = testParameterRepository.findByTestConfigureId(testConfigure.getId());
			...
			if (testParameterService.checkEqutaionExistsForTest(
				...
				if ((testParameter.getInputMethods().equals(InputMethod.OBSERVE)
					...
					if (finishProductParameterResultRepository.findByTestParameterIdAndFinishProductTestCode(
						...
						FinishProductParameterResult finishProductAlready = finishProductParameterResultRepository
			...
			if (finishProductTrial.getTestParameter() != null) {
				...
				if ((testParameter.getInputMethods().equals(InputMethod.CALCULATION)
					...
					finishProductTrial.setValue(
							getFinishProductResultParameter(finishProductCode, testParameter.getTestConfigure().getId(),
	...
	public String getTestEquationFormula(Long testConfigId, Long testParameterId) {
		String testEquation = " ";
		if (testEquationRepository.findByTestConfigureIdAndTestParameterId(testConfigId, testParameterId) != null) {
			testEquation = testEquationRepository.findByTestConfigureIdAndTestParameterId(testConfigId, testParameterId)
	...
	public String getParameterEquationFormula(Long testParameterId, Long configId) {
		...
		if (parameterEquationRepository.findByTestParameterIdAndTestParameterTestConfigureId(testParameterId,
				configId) != null) {
			parameterEquation = parameterEquationRepository
					.findByTestParameterIdAndTestParameterTestConfigureId(testParameterId, configId).getEquation()
					.getFormula();
		}
		return parameterEquation;
	}

	public void updateStatus(String finishProductTestCode, Status status, HttpServletRequest request) {
		FinishProductTest finishProductTest = finishProductTestRepository.findById(finishProductTestCode).get();
	...
	public double getFinishProductResultParameter(String finishProductTestCode, Long testConfigId, Long trialNo,
		...
		try {
			finishProductResult = (double) engine.eval(getParameterEquationFormula(testParameterId, testConfigId));
	...
	public double getFinishProductResult(String finishProductTestCode, Long testConfigId, Long trialNo,
			Long testParameterId) {
		ScriptEngineManager engineManager = new ScriptEngineManager();
		ScriptEngine engine = engineManager.getEngineByName("JavaScript");
		double finishProductResult = 0.0;
		...
		try {
			finishProductResult = (double) engine.eval(getTestEquationFormula(testConfigId, testParameterId));
		} catch (ScriptException e) {
			e.printStackTrace();
		}
		return roundDoubleValue(finishProductResult);
	...
	private int getCountKeyTestPassFinishProductTestsByMaterialSubCategory(String mixDesignCode,
		...
		FinishProductSample finishProductSample = finishProductSampleRepository.findByCode(finishProductSampleCode);
		for (FinishProductTest finishProductTest : finishProductTestList) {
			for (CoreTestConfigure coreTestConfigure : coreTestConfigureRepository
					.findByrawMaterialIdAndCoreTestTrue(finishProductSample.getMixDesign().getRawMaterial().getId())) {
				if (finishProductTest.getTestConfigure().getId() == coreTestConfigure.getTestConfigure().getId())
	...
	private int getCountkeyTestConfigByMaterialSubCategory(Long materialSubCategoryId) {
		return testConfigureRepository.findByMaterialSubCategoryIdAndCoreTestTrue(materialSubCategoryId).stream()
				.filter(listTest -> listTest.getRawMaterial() == null).collect(Collectors.toList()).size();
	}

	private int getCountKeyTestConfigByMaterialCategory(Long materialCategoryId) {
		return testConfigureRepository.findByMaterialCategoryIdAndCoreTestTrue(materialCategoryId).stream()
				.filter(listTest -> listTest.getMaterialSubCategory() == null).collect(Collectors.toList()).size();
	}
	...
	private void checkStatusAndSaveStatus(String finishProductTestCode, HttpServletRequest request) {
		...
		if (mixDesignObj.getStatus().equals(Status.PASS)) {
			MixDesignConfirmationToken mixDesignConfirmationToken = new MixDesignConfirmationToken(mixDesignObj);
			mixDesignConfirmationTokenRepository.save(mixDesignConfirmationToken);
			emailNotification.sendMixdesinApprovelEmail(finishProductSampleobj, mixDesignConfirmationToken, request);
		}
	}

	private void checkPassCountAndTestConfigKeyTestCount(String finishProductTestCode, HttpServletRequest request) {
		FinishProductTest finishproductTest = finishProductTestRepository.findById(finishProductTestCode).get();
		FinishProductSample finishProductSample = finishProductSampleRepository
				.findById(finishproductTest.getFinishProductSample().getCode()).get();
		MixDesign mixDesign = mixDesignRepository.findByCode(finishProductSample.getMixDesign().getCode());
	...
	public void updateFinishProductSampleAndMixDesignStatus(String finishProductTestCode, HttpServletRequest request) {
		FinishProductTest finishproductTest = finishProductTestRepository.findById(finishProductTestCode).get();
	...
	public boolean getEquationCheck(String finishProductTestCode) {
		if (testParameterService.checkEqutaionExistsForTest(finishProductTestRepository.findById(finishProductTestCode)
	...
	private Status checkFinishproductAcceptedValue(Double minValue, Double maxValue, Double value, Condition condition,
	...
	public void checkAcceptedValue(Long testConfigureId, String finishProductTestCode, HttpServletRequest request) {
		FinishProductTest finishProductTest = finishProductTestRepository.findById(finishProductTestCode).get();
		ArrayList<Status> statusList = new ArrayList<Status>();
		if (finishProductTest.getTestConfigure().getAcceptedType().equals(AcceptedType.MATERIAL)) {
			materialAcceptedValueRepository.findByTestConfigureId(testConfigureId).forEach(materialAcceptedValue -> {
				List<FinishProductParameterResult> finishProductResultList = finishProductParameterResultRepository
				...
				if ((materialAcceptedValue.isFinalResult())) {
					finishProductResultList.forEach(result -> {
						if ((materialAcceptedValue.getRawMaterial().getId() == result.getFinishProductTest()
								...
								&& (materialAcceptedValue.getTestParameter().getId() == result.getTestParameter()
										.getId())) {
							statusList.add(checkFinishproductAcceptedValue(materialAcceptedValue.getMinValue(),
	...
	private Long conutrRelavantFinalResult(Long testConfigId, AcceptedType acceptedType, Long rawMaterialId) {
		Long count = (long) 0;
