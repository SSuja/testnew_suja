...
public class FinishProductTrialServiceImpl implements FinishProductTrialService {
	...
	public String saveFinishProductTrial(FinishProductTrial finishProductTrial) {
		if (finishProductTrial.getCode() == null) {
			...
			List<FinishProductTrial> FinishProductTrialList = finishProductTrialRepository.findByCodeContaining(prefix);
	...
	public void updateFinishProductResult(FinishProductTrial finishProductTrial) {
		FinishProductTest finishProductTest = finishProductTestRepository
				.getOne(finishProductTrial.getFinishProductTest().getCode());
		...
		if (finishProductTrial.getTestParameter() != null) {
			...
			if (finishProductTest.getTestConfigure().getDays() == null) {
				...
				finishProductParameterResult.setResult(roundDoubleValue(averageStrength(finishProductTest.getCode())));
	...
	public String getEquation(String finidhProductTestCode) {
		return finishProductTestRepository.getOne(finidhProductTestCode).getTestConfigure().getEquation().getFormula();
	}

	public double averageStrength(String finidhProductTestCode) {
		FinishProductTest finishProductTest = finishProductTestRepository.findById(finidhProductTestCode).get();
		List<FinishProductTrial> finishProductTrialList = finishProductTrialRepository
				.findByFinishProductTestCode(finidhProductTestCode);
	...
	public double calculateFinishProductGradeRatio(FinishProductTrial finishProductTrial, double resultValue,
			double targetValue) {
		HashMap<String, Double> map = new HashMap<>();
		FinishProductTest finishProductTest = finishProductTestRepository
				.findById(finishProductTrial.getFinishProductTest().getCode()).get();
		ArrayList<FinishProductTrialDto> finishProductTrialDtoList = new ArrayList<FinishProductTrialDto>();
		List<TestParameter> testParameterList = testParameterRepository
				.findByTestConfigureId(finishProductTrial.getFinishProductTest().getTestConfigure().getId());
