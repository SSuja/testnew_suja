...
public class ParameterResultController {
	...
	public ResponseEntity<Object> createParameterResult(
			@Valid @RequestBody MaterialParameterResultDto materialParameterResultDto) {
	...
	public ResponseEntity<Object> getAllParameterResults() {
		return new ResponseEntity<>(new ContentResponse<>(Constants.PARAMETER_RESULTS,
				mapper.map(parameterResultService.getAllParameterResults(), ParameterResultResponseDto.class),
	...
	public ResponseEntity<Object> getParameterResultByID(@PathVariable Long id) {
		if (parameterResultService.isParameterResultExist(id)) {
			...
			return new ResponseEntity<>(new ContentResponse<>(Constants.PARAMETER_RESULT_ID,
					mapper.map(parameterResultService.getParameterResultById(id), ParameterResultResponseDto.class),
	...
	public ResponseEntity<Object> UpdateParameterResult(
		...
		if (parameterResultService.isParameterResultExist(parameterResultRequestDto.getId())) {
			parameterResultService.saveParameterValue(mapper.map(parameterResultRequestDto, ParameterResult.class));
	...
	public ResponseEntity<Object> getParameterResultByPlant(@PathVariable String plantCode) {
		if (plantService.isPlantExist(plantCode)) {
			...
			return new ResponseEntity<>(
					new ValidationFailureResponse(Constants.PLANT_ID, validationFailureStatusCodes.getPlantNotExist()),
					HttpStatus.BAD_REQUEST);
		}
	}

	@GetMapping(value = EndpointURI.GET_PARAMETER_RESULTS_BY_MATERIAL_TEST_TRIAL_CODE_AND_MATERIAL_TEST_CODE)
	public ResponseEntity<Object> getParameterResultWithConfigValue(@PathVariable String materialTestTrialCode,
			@PathVariable String materialTestCode) {
		if (materialTestTrialService.isMaterialTestTrialExits(materialTestTrialCode)
				&& materialTestService.isMaterialTestExists(materialTestCode)) {
			return new ResponseEntity<>(
					new ContentResponse<>(Constants.MATERIAL_TEST_TRIAL_CODE,
	...
	public ResponseEntity<Object> getParameterResultByMaterialTestCode(@PathVariable String materialTestCode) {
