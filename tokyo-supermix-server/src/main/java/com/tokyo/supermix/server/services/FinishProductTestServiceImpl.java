...
public class FinishProductTestServiceImpl implements FinishProductTestService {
	...
	public String createFinishProductTest(FinishProductTest finishProductTest) {
		if (finishProductTest.getCode() == null) {
			...
			if (finishProductTestList.size() == 0) {
				...
				finishProductTest.setCode(prefix + String.format("%04d", maxNumberFromCode(finishProductTestList) + 1));
			}
		}
	...
	@Transactional(readOnly = true)
	public List<FinishProductTest> getAllFinishProductTestByPlant(UserPrincipal currentUser, Pageable pageable) {
	...
	@Transactional(readOnly = true)
	public List<FinishProductTest> getAllFinishProductTestByPlant(String plantCode, Pageable pageable) {
		return finishProductTestRepository
				.findByFinishProductSampleMixDesignPlantCodeOrderByUpdatedAtDesc(plantCode, pageable).toList();
	}
	...
	@Transactional(readOnly = true)
	public List<FinishProductTestDto> getFinishProductTestByFinishProductSample(String finishProductSampleCode) {
		ArrayList<FinishProductTestDto> finishProductTestDtoList = new ArrayList<FinishProductTestDto>();
		...
		testConfigureRepository.findByMaterialCategoryIdOrMaterialSubCategoryIdOrRawMaterialId(
				finishProductSample.getMixDesign().getRawMaterial().getMaterialSubCategory().getMaterialCategory()
						.getId(),
				finishProductSample.getMixDesign().getRawMaterial().getMaterialSubCategory().getId(),
				finishProductSample.getMixDesign().getRawMaterial().getId()).forEach(testConfigureMaterial -> {
							...
							.forEach(finishProductTest -> {
								if (finishProductTestRepository.existsByFinishProductSampleCodeAndTestConfigureId(
									...
									if (finishProductTest.getTestConfigure().getAcceptedType() != null) {
										if (finishProductTest.getTestConfigure().getRawMaterial() != null) {
											if (finishProductTest.getTestConfigure().getRawMaterial()
												...
												if (finishProductTest.getTestConfigure().getAcceptedType()
													...
													for (MaterialAcceptedValue materialAcceptedValue : getRawMaterialAcceptedValues(
															finishProductTest.getTestConfigure().getId())) {
														if (materialAcceptedValue.getRawMaterial()
																.getId() == finishProductSample.getMixDesign()
															...
															finishProductTestDto.setTestConfigId(
																	finishProductTest.getTestConfigure().getId());
															finishProductTestDto.setFinishproductTestCode(
																	finishProductTest.getCode());
															finishProductTestDto
																	.setStatus(finishProductTest.getStatus());
															finishProductTestDto.setTestName(finishProductTest
																	.getTestConfigure().getTest().getName());
															finishProductTestDto
																	.setFinishProductSampleCode(finishProductTest
																			.getFinishProductSample().getCode());
															finishProductTestDto.setMainType(
																	finishProductTest.getTestConfigure().getTestType());
															finishProductTestDto.setRawMaterialId(finishProductSample
																	.getMixDesign().getRawMaterial().getId());
										...
										} else if (finishProductTest.getTestConfigure()
												.getMaterialSubCategory() == null) {
											if (finishProductTest.getFinishProductSample().getMixDesign()
													.getRawMaterial().getMaterialSubCategory().getMaterialCategory()
													.getId() == finishProductTest.getTestConfigure()
											...
											if (finishProductTest.getFinishProductSample().getMixDesign()
													.getRawMaterial().getMaterialSubCategory()
													.getId() == finishProductTest.getTestConfigure()
					...
					if (!(finishProductTestRepository.existsByFinishProductSampleCodeAndTestConfigureId(
						...
						if (testConfigureMaterial != null && testConfigureMaterial.getAcceptedType() != null)
							if (testConfigureMaterial.getRawMaterial() != null) {
								if (finishProductSample.getMixDesign().getRawMaterial().getId() == testConfigureMaterial
										.getRawMaterial().getId()) {
									...
									if (testConfigureMaterial.getAcceptedType().equals(AcceptedType.MATERIAL)) {
										for (MaterialAcceptedValue materialAcceptedValue : getRawMaterialAcceptedValues(
											...
											if (materialAcceptedValue.getRawMaterial().getId() == finishProductSample
												...
												finishProductTestDto.setStatus(Status.NEW);
												finishProductTestDto
														.setTestName(testConfigureMaterial.getTest().getName());
												finishProductTestDto
														.setFinishProductSampleCode(finishProductSampleCode);
												finishProductTestDto.setMainType(testConfigureMaterial.getTestType());
												finishProductTestDto.setRawMaterialId(
														finishProductSample.getMixDesign().getRawMaterial().getId());
											}
	...
	@Transactional(readOnly = true)
	public List<FinishProductTest> searchFinishProductTest(BooleanBuilder booleanBuilder, String specimenCode,
			String finishProductSampleCode, String mixDesignCode, String testName, String materialName,
			String plantName, String plantCode, String status, String date, Pageable pageable, Pagination pagination) {
		if (specimenCode != null && !specimenCode.isEmpty()) {
			booleanBuilder.and(QFinishProductTest.finishProductTest.specimenCode.startsWithIgnoreCase(specimenCode));
		}
		...
		if (testName != null && !testName.isEmpty()) {
			booleanBuilder
					.and(QFinishProductTest.finishProductTest.testConfigure.test.name.startsWithIgnoreCase(testName));
		}
		if (materialName != null && !materialName.isEmpty()) {
			booleanBuilder.and(QFinishProductTest.finishProductTest.finishProductSample.mixDesign.rawMaterial.name
					.startsWithIgnoreCase(materialName));
		}
		if (plantName != null && !plantName.isEmpty()) {
			booleanBuilder.and(QFinishProductTest.finishProductTest.finishProductSample.mixDesign.plant.name
					.startsWithIgnoreCase(plantName));
		}
		if (plantCode != null && !plantCode.isEmpty() && !(plantCode.equalsIgnoreCase(Constants.ADMIN))) {
			booleanBuilder.and(QFinishProductTest.finishProductTest.finishProductSample.mixDesign.plant.code
					.startsWithIgnoreCase(plantCode));
		}
		if (status != null && !status.isEmpty()) {
			booleanBuilder.and(QFinishProductTest.finishProductTest.status.stringValue().startsWithIgnoreCase(status));
		}
		if (date != null && !date.isEmpty()) {
			booleanBuilder.and(QFinishProductTest.finishProductTest.date.stringValue().startsWithIgnoreCase(date));
		}
		pagination.setTotalRecords(
				((Collection<FinishProductTest>) finishProductTestRepository.findAll(booleanBuilder)).stream().count());
		return finishProductTestRepository.findAll(booleanBuilder, pageable).toList();
	}

	@Transactional(readOnly = true)
	public List<FinishProductTest> getFinishProductTestsByFinishProductSampleCode(String finishProductSampleCode) {
